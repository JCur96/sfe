...
Here I create a new data frame to perfrom the subsequent operations on in 
...
#   geom_boxplot()
# p6
# one of those would be interesting to have
# p7 <- ggplot(NHM_Pangolins, aes(x=Continent, y=numberOfOverlaps)) +
#   geom_boxplot()
# p7
# probs this one
# p7 <- ggplot(NHM_Pangolins, aes(x=Continent, y=Percent_overlap)) +
#   geom_boxplot()
# p7
# Think this might be quite interesting to include maybe 
p3 <- ggplot(NHM_Pangolins_hulls, aes(x=Continent, y=Percent_overlap)) +
  geom_boxplot()
# p8
# not super uesful actually
# p9 <- ggplot(NHM_Pangolins_hulls, aes(x=binomial, y=Percent_overlap)) +
#   geom_boxplot()
# p9
# # # p3
...
# instead of doing all of the below, just make a table with that information in 
...
#                  family = 'binomial')
# sum_model <- summary(hullModel)
# sum_model$deviance / sum_model$df.resid 
# anova(hullModel, test = "F")
# summary(hullModel)
# 
# # I don't think this is a sensible thing to look at actually. 
...
```
Modelling decade as a predictor of overlap does give a statistically significant
result (p < 0.005, READ UP ON REPORTING LINEAR MODELS), as does modelling distance
as a function of both decade and species, in four cases (REPORT STATS HERE). 
