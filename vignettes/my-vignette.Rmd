...
```{r}
myvars <- c('binomial', 'Decade', 'Percent_overlap', 'distance')
...
p3 <- ggplot(NHM_Pangolins_hulls, aes(x=binomial, y=Percent_overlap)) +
  ...
  scale_x_discrete(labels = abbreviate)
# p
p4 <- ggplot(NHM_Pangolins_hulls, aes(x=as.numeric(Decade), y=Percent_overlap)) +
  geom_point()
# p3 <- ggplot(NHM_Pangolins_hulls, aes(x=binomial, y=distance)) +
#   geom_boxplot() +
#   scale_x_discrete(labels = abbreviate)
# # p3
...
# does not converge
test <- glm(cbind(NHM_Pangolins$numberOfOverlaps, (NHM_Pangolins$numberOfSpecimens - NHM_Pangolins$numberOfOverlaps)) ~ NHM_Pangolins$binomial, family = 'quasibinomial')
# probably massively overdispersed 
test1 <- glm(cbind(NHM_Pangolins$numberOfOverlaps, (NHM_Pangolins$numberOfSpecimens - NHM_Pangolins$numberOfOverlaps)) ~ NHM_Pangolins$Decade, family = 'quasibinomial')
